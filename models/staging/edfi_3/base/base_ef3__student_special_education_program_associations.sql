...
renamed as (
    select
        ...
        {{ jget("v:ideaEligibility::boolean") }}                                    as is_idea_eligible,
        {{ jget("v:iepBeginDate::date") }}                                          as iep_begin_date,
        {{ jget("v:iepEndDate::date") }}                                            as iep_end_date,
        {{ jget("v:iepReviewDate::date") }}                                         as iep_review_date,
        {{ jget("v:lastEvaluationDate::date") }}                                    as last_evaluation_date,
        {{ jget("v:medicallyFragile::boolean") }}                                   as is_medically_fragile,
        {{ jget("v:multiplyDisabled::boolean") }}                                   as is_multiply_disabled,
        {{ jget("v:schoolHoursPerWeek::float") }}                                   as school_hours_per_week,
        {{ jget("v:specialEducationHoursPerWeek::float") }}                         as spec_ed_hours_per_week,
